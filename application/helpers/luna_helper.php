...
if (!function_exists('addRandomCharacters')){
	function addRandomCharacters($str,$length=2) {
	   	$result = '';
	   	$characters = '!#$%^&*()<>?/{}|][-=~';
	   	$ar_characters = str_split($characters);
	   	$characters_length = count($ar_characters)-1;
	   	$ar_str = str_split($str);
	   	for ($i = 0; $i < count($ar_str); $i++) { 
	   		$rand = '';
		   	for ($ii = 0; $ii < $length; $ii++ ) {
		    	$rand .= $ar_characters[floor(((float)rand()/(float)getrandmax()) * $characters_length)];
		   	}
...
if (!function_exists('randomStrName')){
	function findArrayLikeValue($string, $array = array ())
	{
		$res_array = array();
...
if (!function_exists('randomStrName')){
	function randomStrName($str) {
	   	return addRandomCharacters($str.floor(((float)rand()/(float)getrandmax())*1000001)).'_funct';
	}
}  
if (!function_exists('randomNumber')){
	function randomNumber($digits = 7){
