    ...
    function update_checkin_day(){
        if($this->checker(ROLE_DEVELOPER,DEV)==FALSE){
            ...
            $curr_checkin_day = array(
				'id !='=>$id,
				'checkin_day'=>$get_checkin['checkin_day'],
				'checkin_month'=>$get_checkin['checkin_month'],
				'checkin_year'=>$get_checkin['checkin_year'],
			);
			...
			if(!empty($get_curr_checkin_day)){
                ...
                return json(array(
                    'result'=>false,
                    'msg'=>'Checkin day on the month and year already available',
                ));
			}
